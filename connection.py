...
class Bot:
    ...
    def open_browser(self):
        ...
        browser.get(self.base_url + '/mynetwork/invite-connect/connections/')

        browser = self.infinity_scrolling(browser)

        soup = BeautifulSoup(browser.page_source, "html5lib")
        connections = soup.findAll('a', {'class': 'mn-connection-card__link ember-view'})
        print(len(connections))
        for connection in connections:
            current_url = self.base_url + connection['href']
